...
\begin{lemma}
If $p : E \to X$ is a Serre fibration with fiber $F$, then the map
...
Suppose we have produced a lift
\[
\begin{tikzcd}
\partial D^n \arrow[r] \arrow[d] & F \arrow[d] \\
D^n \arrow[r] \arrow[dashed, ur] & E \times_X PX.
\end{tikzcd}
\]

%I didn't quite follow this argument, but maybe it will be repeated Monday?

Note that a map $D^n \to PX$ is equivalent to a map $D^n \times I \to X$. Therefore a map $D^n \to E \times_X \to PX$ is equivalent to the non-blue data in the diagram. Since $\partial D^n$ lands in the fiber...
