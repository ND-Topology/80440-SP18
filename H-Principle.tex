...
Many kinds of structures on a manifold $M$ can be described as $G$-structures on its tangent bundle for a Lie group $G$. For example, an orientation on $M$ is equivalent to giving an $SO(m)$-structure, for $m=\dim M$. Unfortunately, foliations are more subtle and aren't described by a Lie group. Fortunately, though, a foliation of codimension $q$ can be described by a certain Lie \textit{groupoid}, $\Gamma^q$, which we now construct.
