...
For any smooth manifold $X,Y$ and positive integer $k$, there is an associated bundle $J^k(X,Y)\to X\times Y$ called the k-jets from $X$ to $Y$. As motivation, we recall two definitions of the tangent bundle, which we will see is a special case $TM=J^1_0(\R, M)$.

%$J^1_0(\R, M)$ 
For $M$ a smooth manifold, $TM$ can be defined as the equivalence classes of curves $(\alpha, B_{\epsilon}(0))$ where $(-\epsilon, \epsilon) \xrightarrow{\alpha} M$. Then there are multiple ways to define the equivalence relation on these curves. Suppose $(\alpha, B_{\epsilon_1}(0))$ and $(\beta, B_{\epsilon_2}(0))$ have $\alpha(0)=\beta(0)$. These two are equivalent if, for some coordinate chart $X: \R^n\to M$, we have $(X^{-1}\circ \alpha)'(0) = (X^{-1}\circ \beta)'(0) \in \R^n$. With an eye towards coordinate independence, we can instead view $\alpha$ as defining derivation on $C^\infty_x(M)$ of germs of smooth function near $x=\alpha(0)\in M$ as
\[
...
\begin{remark}
One can define extend the definition of k-jets to any smooth fiber bundle $E\to X$ with fiber $Y$, such that sections $J^k(X\times Y)=J^k(X,Y)$. (See [Michor: Topics in Differential Geometry]).
\end{remark}
...
\begin{definition}
Given $f: X \to Y$, there is an induced section $j^kf: X \to J^k(X,Y)$, given by $x \mapsto (x,f, X)$ called the \textit{$k$-th jet prolongation} of $f$.
\end{definition}
\begin{definition}
A section $s: X\to J^k(X,Y)$ is called \textit{holonomic} if $s=j^kf$ for some $f:x\to Y$. 
\end{definition}
In other words, a section is holonomic if the various higher derivatives are compatible and combine to an actual function. But in general, the space of sections $X\to J^k(X,Y)$ is much bigger than the space of functions $X\to Y$.
...
\begin{definition}
A \textit{partial differential relation} or sometimes \textit{differential relation} of order $k$ is a subset $R \subset J^k(X,Y)$.
A function $f:X \to Y$ is said to be a \textit{solution} to $R$ or satisfy $R$ if $j^k f(x) \in R$ for all $x \in X$.
\end{definition}
Now we will give explicit relations for a few types of maps $X\to Y$. First, we need to understand the $k=1$ jet bundle more explicity. First note that the fiber of the bundle $J^1(X,Y)\to X\times Y$ over the point $(x,Y)$ is equal to $\Hom(T_xX, T_yY)$, where $f:X\to Y$ maps to $df: T_xX\to T_yY$. These fibers piece together as
\[
J^1(X,Y) = T^*X\otimes TY
\]
as bundles over $X\times Y$. Thus we can statify $J^1(X,Y)$ as
\[
