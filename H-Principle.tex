...
Note that $f\circ \tilde{\gamma}|_{S^n}\simeq f\circ \alpha$, so by $f_*:\pi_nX \to \pi_nY$ injective $\tilde{\gamma}|_{S^n}\simeq \alpha$, pick such a homotopy $H:S^n\times I \to X$, such that $H(-,1)=\alpha$, and $H(-,0)=\tilde{\gamma}|_{S^n}$. 
So we can construct $\overline{\beta}: D^n\to X$, where when $|x|>\frac{1}{2}$ we use $H(\frac{x}{|x|}, 2|x|)$, and when $|x| \leq \frac{1}{2}$, we rescale $\tilde{\gamma}$ restricted to the hemisphere that under $f$ gets homotoped to the hemisphere corresponding to $\beta$.
